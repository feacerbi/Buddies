...
import com.buddies.pet.model.OpenMultipleDocumentsWithPersistedPermissions
...
class PetProfileFragment : NavigationFragment(), CoroutineScope {
    ...
    val galleryAdapter = GalleryAdapter(this@PetProfileFragment)
    ...
    private fun setUpViews() = with (binding) {
        ...
        toolbar.setOnMenuItemClickListener {
            when (it.itemId) {
                ...
                R.id.add_gallery_menu_action -> {
                    multipleGalleryPick.launch(arrayOf(IMAGE_MIME_TYPE))
                    true
                }
                else -> false
            }
        ...
        petGallery.adapter = galleryAdapter
    }
    ...
    private fun bindViews() = with (binding) {
        observe(viewModel.getStateStream()) {
            ...
            galleryAdapter.updateItems(it.gallery)
