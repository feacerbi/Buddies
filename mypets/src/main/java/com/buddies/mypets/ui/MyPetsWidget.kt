...
class MyPetsWidget @JvmOverloads constructor(
    ...
    private val useCases by inject(MyPetsUseCases::class.java)
    ...
    private fun addPets(
        ...
        val pets = useCases.getPetsFromCurrentUser()

        if (isAttachedToWindow) {
            adapter = MyPetsAdapter(pets, owner = lifecycleOwner) {
                Toast.makeText(context, it.info.name, Toast.LENGTH_SHORT).show()
            }
